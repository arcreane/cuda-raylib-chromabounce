...
__global__ void initParticlesKernel(Particle* particles, int numParticles) {
    ...
    if (idx < numParticles) {
        particles[idx].x = idx % 100 + 50; // Position X
        particles[idx].y = idx / 100 + 50; // Position Y
...
__global__ void updateParticlesKernel(Particle* particles, int numParticles, float interactionRadius, float forceStrength, float timeStep) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        updateParticlesKernel << <blocksPerGrid, threadsPerBlock >> > (d_particles, numParticles, interactionRadius, forceStrength, timeStep);
        cudaDeviceSynchronize();
        ...
        for (int i = 0; i < numParticles; ++i) {
            Color particleColor = { (unsigned char)h_particles[i].color, 50, 200, 255 };
